...
import { Message, Employee, StaffMessage } from '../types';
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg ${mode === 'email' ? 'bg-slate-900 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                Email
              </button>
              <button
                onClick={() => setMode('direct')}
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg ${mode === 'direct' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                Direct
              </button>
              {mode === 'email' && (
                          ...
                          {Array.isArray(parsedAttachments) && parsedAttachments.length > 0 && (
                            <div className={`rounded-lg ${isOwn ? 'bg-blue-500/30' : 'bg-white/60'} p-2 text-xs space-y-1`}>
                              {parsedAttachments.map((att: any, idx: number) => (
                                <a key={idx} href={att.filePath || att.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 underline">
